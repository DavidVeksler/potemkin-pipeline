...
const bossEl=$('#boss'), settingsEl=$('#settings'), helpEl=$('#help'), toastEl=$('#toast'), liveBtn=$('#livebtn'), cfgbtn=$('#cfgbtn');
...
let bossActive=false, bossFrame=0, settingsOpen=false, helpOpen=false;
...
function buildConfig(){
  ...
  let g;
  g=sec('Identity');
  fld(g,'agent',txt(cfg.agent,v=>{cfg.agent=v;hAgent.textContent=v||'HYPERION';syncURL();}),true);
...
function urlParams(forceSeed){
  ...
  if(cfg.agent!=='HYPERION')p.set('agent',cfg.agent);
...
function resetConfig(){
  cfg.agent='HYPERION'; cfg.model='mythos-5-preview'; cfg.audio='off'; cfg.crt='off'; cfg.reduceFlash=null;
