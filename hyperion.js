...
const SCENE_GROUPS=[
  {title:'Observability & telemetry', items:[
    ...
    ['btop','btop · runaway process'],['anomaly','metric anomaly'] ]},
  {title:'Performance & profiling', items:[
    ['flame','pprof flame graph'],['sql','EXPLAIN · slow query'],
    ['load','k6 load test'],['heatmap','latency heatmap'] ]},
...
/* ====================================================================== */
let actx=null, master=null, audioConfirmed=false;
function audioUnlock(){
  if(cfg.audio!=='on')return;
  try{
    if(!actx){ actx=new (window.AudioContext||window.webkitAudioContext)(); master=actx.createGain(); master.gain.value=0.16; master.connect(actx.destination); }
...
function beep(kind){
  ...
  if(kind==='alert') klaxon();
  else if(kind==='deploy') fanfare();
  else if(kind==='ok') resolveChord();
}
...
function sfxOverlay(ev,now){
  switch(ev.op){
    case 'open': case 'app': sweep(120,900,0.5,'sawtooth',0.22); noise(0.4,0.06,200); break;   // tension riser as the overlay slams open
    case 'banner': if(ev.cls&&ev.cls.indexOf('ok')>=0) resolveChord(); break;
